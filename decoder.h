 ...
 * 解码器 (Decoder)
 * 
 * Transformer 的解码器部分，用于生成目标语言序列
 * 
 * 解码器层结构：
 * - Multi-Head Self-Attention: 自注意力机制（带掩码）
 * - Multi-Head Cross-Attention: 交叉注意力机制（关注编码器输出）
 * - Positionwise Feed-Forward: 前馈神经网络
 * - Sublayer Connection: 残差连接和层归一化
 * 
 * 解码器由 N 个相同的解码器层堆叠而成，每层包含：
 * 1. Masked Self-Attention 子层（带残差连接和层归一化）
 * 2. Cross-Attention 子层（关注编码器输出，带残差连接和层归一化）
 * 3. Feed-Forward 子层（带残差连接和层归一化）
